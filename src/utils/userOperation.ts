...
export const createSmartAccountClient = async (signer: SmartAccountSigner, accountAddress?: Address) => {
  ...
  return createSmartAccountClientAlchemy({
    ...
    paymasterAndData: {
      ...
      paymasterAndData: async (userOp) => {
        ...
        const resolvedUserOp = (await resolveProperties(userOp)) as UserOperationStruct_v6
        // request sponsorship
...
const formatAsHexOptional = (value?: string | Uint8Array | bigint | number): Hex | undefined => {
  if (value === undefined) {
    return undefined
  } else {
