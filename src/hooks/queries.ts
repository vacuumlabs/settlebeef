...
export const useBeef = (address: Address): (Beef & { refetch: () => Promise<unknown> }) | null | undefined => {
...
export const useGetInfiniteBeefs = (pageSize: number) => {
  ...
  return useInfiniteQuery({
    ...
    getNextPageParam: (_1, _2, lastPageParam) => {
      ...
      if (beefsLength === undefined || nextPageParam * pageSize >= beefsLength) return null
...
export const useGetArbiterStatuses = (beefAddress: Address, arbiters: readonly Address[]) => {
  ...
  if (data) {
    ...
    const chunkedData = Array.from({ length: Math.ceil(data.length / 2) }, (_, index) =>
      data.slice(index * 2, index * 2 + 2),
    )

    const groupedData: ArbiterStatus[] = chunkedData.map(([hasAttended, hasSettled], index) => {
      return {
