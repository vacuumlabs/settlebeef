...
export const generateAddressForEmail = async (email: string) => {
  const { rows } =
    await sql<UserDetailsResponseType>`SELECT smart_account_address FROM user_details WHERE email = ${email} AND chain_id = ${activeChain.id}`;
  ...
  await sql`INSERT INTO user_details (email, temporary_private_key, smart_account_address, chain_id) 
      values (${email}, ${privateKey}, ${accountAddress}, ${activeChain.id})`;
